import type { IProblem, IPathFormatter } from "../../../api/index.js";
...
export class Problem implements IProblem<State, Operator> {
  ...
  solve(algoName: string, verbose: boolean): string {
    ...
    if (name === "BFS") {
      ...
      algo = new AStar(this, heuristic, verbose);
    } else if (name === "IDA*" || name === "IDASTAR") {
      algo = new IDAStar(this, heuristic, verbose);
    } else if (name === "DFBNB") {
      algo = new DFBnB(this, heuristic, verbose);
    } else {
